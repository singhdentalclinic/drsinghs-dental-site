  ...
  return (
    ...
    >
      <div className="w-full">
        <div className="flex items-center justify-between h-12 px-6 xl:px-12">
          {/* Logo */}
          <Link
            href="/"
          ...
          >
            <div className="relative w-6 h-6 md:w-9 md:h-9 group-hover:scale-110 transition-transform duration-300">
              <Image
            ...
            <div className="flex flex-col font-monotype-corsiva pr-2 md:pr-4">
              <span className="text-lg font-bold leading-tight text-primary">Dr. Singh&apos;s</span>
          ...
          {/* Desktop Navigation */}
          <nav
            className={`hidden xl:flex items-center space-x-2 px-2 py-1.5 rounded-full transition-all duration-300 ${
          ...
          >
            {navigationItems.map((item) => (
              ...
              >
                <Link
                  href={item.path}
                  className="px-2.5 py-1 font-body text-[13px] font-medium rounded-full transition-all duration-200 text-gray-700 hover:text-primary hover:bg-gray-50 flex items-center space-x-1"
                >
                  ...
                  <AnimatePresence>
                    {activeDropdown === item.label && (
                      <motion.div
                        ...
                        className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-44 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50 overflow-hidden"
                      >
                        {item.subItems.map((subItem) => (
                          <Link
                            key={subItem.path}
                            href={subItem.path}
                            className="block px-3 py-1.5 text-sm font-medium text-gray-600 hover:text-primary hover:bg-gray-50/80 hover:underline underline-offset-4 decoration-primary/30 transition-all border-b border-gray-200 last:border-0"
          ...
          <div className="hidden xl:flex items-center">
            <Link
              ...
              className="cursor-pointer transition-all bg-blue-500 text-white px-4 py-1 rounded-lg border-blue-600 border-b-[4px] hover:brightness-110 hover:-translate-y-[1px] hover:border-b-[6px] active:border-b-[2px] active:brightness-90 active:translate-y-[2px] text-[15px] font-bold"
            >
              Call us
            </Link>
            <div
        ...
        >
          <div className="p-4 space-y-2 flex flex-col">
                      ...
                      <div className="pl-4 pb-2 space-y-1">
                        {item.subItems.map((subItem) => (
                          <Link
                            key={subItem.path}
                            href={subItem.path}
                            className="block py-1.5 text-sm text-gray-500 hover:text-primary hover:underline underline-offset-4 decoration-primary/30 transition-all border-b border-gray-200 last:border-0"
                            onClick={() => {
            ...
            <div className="pt-4 pb-2 flex flex-col space-y-3">
              <Link
                href="tel:+918449830107"
                className="w-full cursor-pointer transition-all bg-blue-500 text-white px-6 py-2.5 rounded-xl border-blue-600 border-b-[4px] hover:brightness-110 hover:-translate-y-[1px] hover:border-b-[6px] active:border-b-[2px] active:brightness-90 active:translate-y-[2px] text-center font-bold"
                onClick={() => setIsMobileMenuOpen(false)}
              >
                Call us
              </Link>
